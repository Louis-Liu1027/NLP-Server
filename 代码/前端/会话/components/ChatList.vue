<template>
  <div>
    <section class="chatlist" :class="showSelBox>0?'chatlist-bottom-collapse':'chatlist-bottom'">
      <mt-loadmore :top-method="loadTop" top-pull-text="加载更多" top-drop-text="释放加载" @top-status-change="handleTopChange"
                   ref="loadmore">
        <ul>
    ...
    <section class="foot">
      ...
      <span class="btn-plus" v-on:click="showSelBox=showSelBox==2?0:2"><i class="fa" aria-hidden="true"
                                                                          :class="showSelBox==2?'fa-minus-circle':'fa-plus-circle'"></i></span>
  ...
  export default {
    ...
    data () {
      return {
        ...
        records: [{
          ...
          time: util.formatDate.format(new Date(), 'yyyy-MM-dd hh:mm:ss'),
          name: '智能客服',
          content: '您好，智能客服为您服务，请按照以下格式进行提问：如哪家商家的商品类型有什么，某种类型的商品品类含有哪些商品，商品的信息\n'
        }],
        // 表情
        EXPS: [
    ...
    methods: {
      ...
      sendMsg: function () {
        ...
        this.$axios({
          method: 'get',
          url: 'http://127.0.0.1:8081/hello', //发出请求的目标路径
          params: {//发送输入的值
            code: this.content
          }
        }).then(function (res) {
          output = res.data//获取到的结果
        }).catch(function (err) {
          console.log(err)
        })
        setTimeout(function () {
